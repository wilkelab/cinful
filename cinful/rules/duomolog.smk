...
SAMPLES, = glob_wildcards("{config[outdir]}/01_orf_homology/{sample}")
...
# 		input_seqs = "{config[outdir]}/01_orf_homology/{component}/filtered_nr.fa"
# 	output:
# 		"{config[outdir]}/01_orf_homology/{component}/blast.txt"
# 	shell:
...
# 		input_seqs = "{config[outdir]}/01_orf_homology/{component}/{sample}.filtered.fa",
# 		blastOut = "{config[outdir]}/01_orf_homology/{component}/blast.txt"
# 	output:
...
# rule merged_results:
# 	input:
# 		blast_v_hmmer = expand("{config[outdir]}/01_orf_homology/{component}/blast_v_hmmer.csv",
...
# 			componentDFs.append(pd.read_csv(componentHomologFile, 'a'))
# 		mergedDf = pd.concat(componentDFs)
# 		mergedDf.to_csv(output, index = None)
